...
### Two URLs, two jobs — do not unify them

The same `KrightSetup-X.Y.Z.exe` is reachable at two addresses, deliberately:
...
The appcast URL is baked into `appcast-win.xml` and generated by
`scripts/gen-appcast.ps1` (`$dlPrefix`), and each enclosure carries an Ed25519
signature over that exact file. **Do not repoint the appcast at the blob** to make
the two match — it would invalidate the signed enclosure and break updates for
users already on the current version. Both URLs are versioned and immutable, so
serving the Store from blob costs nothing.
