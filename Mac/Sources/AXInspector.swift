...
enum AXInspector {
    ...
    /// Most recent focused element that wasn't Kright itself, kept so we can write
    /// back even after Kright steals focus. AX value-setting doesn't require focus.
    ...
    /// Returns nil if not trusted, no element, or the element is Kright itself.
    ...
    /// Reads the currently focused (non-Kright) element and its text value, fresh —
    ...
    /// Writes `text` into the last focused (non-Kright) field. Returns success + error.
